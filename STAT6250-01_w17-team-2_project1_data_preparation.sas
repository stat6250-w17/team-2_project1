...
Dataset Name: CAASP Test Results for English language arts/literacy and 
              Mathematics for Fremont Unified School District for Alameda 
              county for years 2015 and 2016 (2 separate datasets)

Experimental Units: Test result for each type of test for each school in 
                    Alameda Number of Observations 14431 (2015) and 
                    13952 (2016)

Number of Features: 33
Data Source: The files CAASP_Test_ results_2015_2016.xls has been created by 
             downloading and combining the datasets below into a single xls
...
Notes for creating the xls file: 
	- Download the test data (CSV format) for year 2015 & 2016. 
        - Replace all '*' in data with a blank.
	- Also download the Entity(schools) data and, the lookup tables 
          Test and Subgroups data for each year. Add an Area N key to 
          explain the various Area n scores in teh dataset.


Data Dictionary: The column names are self-explanatory in the data and 
          can be seen when opened in Excel. Descriptions for the ‘Area n’ 
          fields can be obtained at 
          http://caaspp.cde.ca.gov/sb2016/UnderstandingCAASPPReports

Unique ID:  The columns ‘CountyCode + DistrictCode + SchoolCode + TestYear + 
            SubgroupID + Grade + Test ID’ together form the primary key.
;
...
filename CAASPtmp clear; /*clear the fileref created for the input xls file*/

/* Using Proc SQL, create an analytic dataset that will be used for each of the
   analysis questions. This query uses the SQL statement to combine(using 
   UNION) data for years 2015 and 2016 and pulls in the subgroup, test, 
   school name etc lookup worksheets to provide all necessary columns in a 
   single dataset  named 'CAASP1516_analytic_file'. 
...
Select 
	...
	a.Percentage_Standard_Met_and_Abov as pcnt_met_and_above,
